...
if(isset($yazi_ekle)){
    ...
    if($kaynak){
        if(!in_array($uzanti,$dosyauzanti) && !in_array($turu,$dosyaturu)){
            header("location: articles.php?update=gecersizuzanti");

        }elseif($boyut>1024*1024){
            header("location: articles.php?update=buyuk");
        }else{
            if(move_uploaded_file($kaynak,$hedef)){
                $yükle=$db->prepare("INSERT INTO yazilar (yazi_foto,yazi_title,yazi_kategori_id,yazi_icerik) VALUES (?,?,?,?)");
                $insert=$yükle->execute(array($resimAd,$yazi_title,$yazi_kategori,$yazi_icerik,));
                if($insert){
                    header("location: articles.php?update=yes");
                }
                else{
                    header("location: articles.php?update=no");
                }
            }
        }
    }
    ...
    $hedef="../blog/images/".$resimAd;
        if($kaynak){
            if(!in_array($uzanti,$dosyauzanti) && !in_array($turu,$dosyaturu)){
                ...
                if(move_uploaded_file($kaynak,$hedef)){
                    $yükle=$db->prepare("UPDATE yazilar SET yazi_foto=?, yazi_title=? , yazi_kategori_id=?, yazi_icerik=? WHERE yazi_id=?");
                    $update=$yükle->execute(array($resimAd,$yazi_title,$yazi_kategori,$yazi_icerik,$yazi_id));
                    if($update){
                        header("location: articles.php?update=yes");
                    }
                    else{
        ...
        if(!$yazi_title || !$yazi_icerik || !$yazi_kategori){
            header("location: articles.php?update=empty");
        }else{
            $yükle=$db->prepare("UPDATE yazilar SET yazi_title=? , yazi_kategori_id=?, yazi_icerik=? WHERE yazi_id=?");
            $update=$yükle->execute(array($yazi_title,$yazi_kategori,$yazi_icerik,$yazi_id));
            if($update){
                header("location: articles.php?update=yes");
            }else{
                header("location: articles.php?update=no");
...
if(isset($_POST['kategori_ekle'])){
    if(!$kategori_title){
    header("location: categories.php?sonuc=empty");
    }
    else{
    ...
    $insert=$kategoriler->execute(array($kategori_title));
        ...
        if($insert){
            ...
            header("location: categories.php?update=no");
        }
    }
}
...
if(isset($_POST['kategori_duzenle'])){
    ...
    if(!$kategori_title){
    header("location: categories.php?update=empty");
    }
    else{
...
if(isset($_POST['yorum_duzenle'])){
    ...
    $update=$yorumlar->execute(array($yorum_icerik,$yorum_durum,$yorum_id));
        ...
        if($update){
            header("location: comments.php?update=yes");
        }
        else{
            header("location: comments.php?update=no");
        }
}
...
if(isset($_POST['yorum_cevapla'])){
    ...
    header("location: answers.php?sonuc=empty");
    }
    else{
    ...
    $insert=$yorumlar->execute(array($yorum_yapan,$yorum_mail,$yorum_website,$yorum_yazi_id,$yorum_icerik,1,$yorum_id));
        ...
        if($insert){
            ...
            header("location: answers.php?update=yes");
        }
        else{
